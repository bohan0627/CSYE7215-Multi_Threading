...
public class Validate {
	...
	public static boolean validateSimulation(List<SimulationEvent> events) {
		try {
			...
			/**
			 * Calculate the average waiting time for each given priority
			 */
			System.out.println("Average waiting time based on the priority:");
			for(int i=1;i<=3;i++){
				if(CoffeeShop.customerCountByPriority[i]!=0)
					System.out.println("The average waiting time for priority " + i + " is: " + CoffeeShop.waitingTimeByPriority[i] / CoffeeShop.customerCountByPriority[i] + "ms");
				else
					System.out.println("This priority has no counts");
			}
			return true;
