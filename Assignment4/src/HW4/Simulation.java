...
public class Simulation {
	...
	public static List<SimulationEvent> runSimulation(
		...
		logEvent(SimulationEvent.startSimulation(numCustomers,
				...
				numTables,
				machineCapacity));
		CoffeeShop coffeeShop = new CoffeeShop(numCustomers, numCooks, numTables, machineCapacity, randomOrders);
		coffeeShop.runCoffeeShop();
	...
	public static void main(String args[]) throws InterruptedException {
		...
		int numCumers = new Integer(args[0]).intValue();
		...
		/**
		 * This part for printing the sequences of placing order and processing order
		 * Please use if possible
		 */
		/*
		CoffeeShop coffeeShop = CoffeeShop.getInstance();
		List<Integer> place = coffeeShop.getPlaceList();
		List<Integer> start = coffeeShop.getStartList();
		System.out.println("The sequence of placing order:");
		for(Integer i: place)
			System.out.println(i);
		System.out.println("The sequence of processing order:");
		for(Integer i: start)
			System.out.println(i);
		*/
	}
